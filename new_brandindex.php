...
<?php
    ...
    for ($i=0; $i<4; $i++) {
        for ($j=1; $j<4; $j++) {
            for ($k=1; $k<3; $k++) {
                $nick_name = 'ser_res_'.$i.'_'.$j.'_'.$k;
                $_SESSION[$nick_name] = 0; // Session init for Json Data
            }
        }
    }
    ...
    function amazon($domResult) {
        ...
        if ($domResult->find('span.a-offscreen',0)) {
            ...
            if (isset($_POST['ser_key1']) && strlen($_POST['ser_key1']) > 0) {
                if ($_SESSION['ser_res_3_1_1']==0 && $_SESSION['ser_res_3_1_2']==0) {
                    if (strlen($_POST['ser_key1'])>0 && $_SESSION['ser_res_3_1_1']==0) {
                        $_SESSION['ser_res_3_1_1']=$matchesRes1[0];
                    }
                    if (strlen($_POST['ser_key1'])>0) {
                        $ret = manageIconStr($part);
                        $_SESSION['ser_res_3_1_2'] = $ret;
                    }
                    return;
                }
            }
            if (isset($_POST['ser_key2']) && strlen($_POST['ser_key2']) > 0) {
                if ($_SESSION['ser_res_3_2_1']==0 && $_SESSION['ser_res_3_2_2']==0) {
                    if (strlen($_POST['ser_key2'])>0) {
                        $_SESSION['ser_res_3_2_1']=$matchesRes1[0];
                    }
                    if (strlen($_POST['ser_key2'])>0) {
                        $ret = manageIconStr($part);
                        $_SESSION['ser_res_3_2_2'] = $ret;
                    }
                    return;
                }
            }
            if (isset($_POST['ser_key3']) && strlen($_POST['ser_key3']) > 0) {
                if ($_SESSION['ser_res_3_3_1']==0 && $_SESSION['ser_res_3_3_2']==0) {
                    if (strlen($_POST['ser_key3'])>0)  {
                        $_SESSION['ser_res_3_3_1']=$matchesRes1[0];
                    }
                    if (strlen($_POST['ser_key3'])>0 && $_SESSION['ser_res_3_3_2']==0) {
                        $ret = manageIconStr($part);
                        $_SESSION['ser_res_3_3_2'] = $ret;
                    }
                    return;
                }
            }
    ...
    function twitter($domResult) {
        // There is a captcha/recaptcha in this site. Can't scrape using curl
    }
